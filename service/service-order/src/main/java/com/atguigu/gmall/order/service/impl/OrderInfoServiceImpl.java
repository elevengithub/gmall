...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo>
    ...
    @Transactional
    @Override
    public Long saveOrder(String tradeNo, OrderSubmitVo vo) {
        //1、准备订单数据
        OrderInfo orderInfo = prepareOrderInfo(tradeNo,vo);
        //2、保存orderInfo
        orderInfoMapper.insert(orderInfo);
        //3、准备OrderDetail
        List<OrderDetail> details = prepareOrderDetail(orderInfo,vo);
        //4、保存OrderDetail到数据库
     ...
     * @return
     */
    private List<OrderDetail> prepareOrderDetail(OrderInfo orderInfo, OrderSubmitVo submitVo) {
        List<OrderDetail> list = submitVo.getOrderDetailList().stream()
                .map(vo -> {
     ...
     * @return 订单类OrderInfo
     */
    private OrderInfo prepareOrderInfo(String tradeNo, OrderSubmitVo submitVo) {
